...
**Windowsのアプリ版なら、設定画面のスライダーで変えられます。**

1. タスクトレイの Ollama アイコン → **Settings**
2. **Context length** のスライダーを **`16k`** まで動かす
3. そのまま閉じる（保存ボタンはありません）

`4k` のままだと、マニュアルの大半が捨てられた状態で回答されます。
`32k` 以上にしてもこの用途では効果が薄く、VRAMを余計に使うだけなので `16k` で十分です。
（根拠ページ数を増やしたくなったら `32k` にしてください）

コマンドで設定する場合はこちらです。アプリ版のスライダーを使うならこれは不要です。
